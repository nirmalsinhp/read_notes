...
- can be used for type deduction for function return value based on template params & operations
...
- lambda can be stored in std::function, but it is slower than storing it in closure using auto.
...
- c++11 cbegin() factory method, gives const iterators for non-const containers also, and const iterators works everywhere even in insert, erase as thet are just providing positions.
...
- no shared_ptr<T[]> array variant -- in c++17, array variant introduced, 
...
- std::forward is conditional cast - cast to rvalue if and only if args passed is rvalue itself.
...
- function taking universal references are greediest of all functions, almost all calls match with this.
